...
acccon = r"C:\Users\kinse\Downloads\Main_AccountsContacts - Contacts Main.csv"
#-----------------------------------------------------------------------------------
...
class CallLogWindow:
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        #Tools - Collection of useful calcs/docs/etc
        #--------------------------------------------------------------------------------------------------------
        self.toolmenu = tk.Menu(self.calllog_menu, tearoff = 0, background = '#0C1021', foreground = 'white', activebackground = 'dark goldenrod', activeforeground = 'white')
        self.toolmenu.add_command(label = "Calculators", command = lambda: None)
        self.calllog_menu.add_cascade(label = "Tools", menu = self.toolmenu) 
        ...
        self.bottomframe = tk.Frame(self.parent, bg = BG)
        self.bottomframe.grid(row = 1, column = 0, columnspan =5, sticky = tk.EW)       
        ...
        self.mon_labelframe = tk.LabelFrame(self.display_frame, text = 'MONDAY', 
                                            bg =  BG,  fg =  FG, font = fsmall, cursor = 'hand2')
        ...
        #To DO labelframe
        #---------------------------------------------------------------------------------------------------------
        self.todo_lab = tk.Label(self.todo_frame, text = "CALENDAR", bg = BG, fg = FG,
                                 font = fbig)
        self.todo_lab.grid(row = 0, padx = 5, pady = 5)
        
        self.cal = Calendar(self.todo_frame, font= fsmall, selectmode='day', locale='en_US',
        ...
        self.todo_lframe = tk.LabelFrame(self.todo_frame, text = 'ToDo - TrelloClone',
                                         bg = BG, fg = FG, font = fsmall, cursor = 'hand2')
        ...
        self.todo_text1 = tk.Text(self.frame1, height = 27, width = 70, bg = BG, fg = FG, yscrollcommand = self.todo_scroll1.set,
                                 selectbackground = DGR, selectforeground = 'black',  font = fsmall)
        ...
        self.todo_text2 = tk.Text(self.frame2, height = 27, width = 70, bg = BG, fg = FG, yscrollcommand = self.todo_scroll2.set,
                                 selectbackground = DGR, selectforeground = 'black',  font = fsmall)
        ...
        self.todo_text3 = tk.Text(self.frame3, height = 27, width = 70, bg = BG, fg = FG, yscrollcommand = self.todo_scroll3.set,
                                 selectbackground = DGR, selectforeground = 'black',  font = fsmall)
        ...
        self.edit_entry_button = tk.Button(self.display_frame, text = 'Edit Entries', bg = BG, 
                                           fg = FG, cursor = 'hand2', font = fsmall)
        ...
        self.spinbox = tk.Spinbox(self.det_labelframe_1, values = self.choices, bg =  '#0C1021', fg = DGR,
                                  font = fsmall, cursor = 'hand2', justify = tk.CENTER, textvariable = self.cb)
        ...
        #Company name-------------------------------
        self.complab = tk.Label(self.det_labelframe_1, text = 'Company Name', bg = BG, fg = FG, font = fsmall)         #some type of auto populate
        self.complab.grid(row = 1 , column = 1, padx = 3, pady = 3, sticky = tk.W)
        self.comp_ent =  tk.Entry(self.det_labelframe_1, bg = BG, fg = FG, font = fsmall, width = 50,
        ...
        #Contact Info------------------------------------
        self.contlab = tk.Label(self.det_labelframe_1, text = 'Contact Name', bg = BG, fg = FG, font = fsmall)
        self.contlab.grid(row = 2 , column = 1, padx = 3, pady = 3, sticky = tk.W)
        self.cont_ent =  tk.Entry(self.det_labelframe_1, bg = BG, fg = FG,  font = fsmall, width = 50,
                               cursor = 'xterm', selectbackground = 'dark goldenrod', insertbackground = 'white' )
        ...
        self.medlab = tk.Label(self.det_labelframe_1, text = 'Medium Used', bg = BG, fg = FG, font = fsmall)           #Think about using a spinbox
        self.medlab.grid(row = 3 , column = 1, padx = 3, pady = 3, sticky = tk.W)
        self.med_ent =  tk.Entry(self.det_labelframe_1, bg = BG, fg = FG, font = fsmall, width = 50,
                              cursor = 'xterm', selectbackground = 'dark goldenrod', insertbackground = 'white' )
        ...
        self.desclab = tk.Label(self.det_labelframe_1, text = 'Description', bg = BG, fg = FG, font = fsmall)
        ...
        self.desc_text = tk.Text(self.det_labelframe_1, bg = BG, fg = FG, font = fsmall, height = 5, width = 50,
        ...
        self.fwdlab = tk.Label(self.det_labelframe_1, text = 'Forward', bg = BG, fg = FG, font = fsmall)
        ...
        self.fwd_text = tk.Text(self.det_labelframe_1, bg = BG, fg = FG, font = fsmall, height = 5, width = 50,
                                cursor = 'xterm', selectbackground = 'dark goldenrod', selectforeground = 'black', insertbackground = 'white')
        ...
        #BUTTON WIDGET -----------------------------------------------------------------------------------------------------
        self.submitbutton = tk.Button(self.det_labelframe_1, bg = BG, fg = FG, font = fsmall, cursor = 'hand2',
        ...
        self.det_text = tk.Text(self.det_labelframe_2, bg = BG, fg = FG, font = fsmall, height = 20, width = 65, yscrollcommand = self.det_scroll.set,
                                cursor = 'xterm', selectbackground = 'dark goldenrod', selectforeground = 'black', insertbackground = 'white')
        self.det_text.grid(row = 0, column = 0, padx = 5, pady = 5, sticky = tk.NS) 
        
        self.det_scroll.config(command = self.det_text.yview)
        ...
        self.oth_months_instruct = tk.LabelFrame(self.det_labelframe_2, text = 'See other Weeks Info', bg = BG, fg = FG)
        self.oth_months_instruct.grid(row = 1, padx = 3, pady = 3, sticky = tk.EW)
        
        self.datepick = DateEntry(self.oth_months_instruct, locale='en_US', date_pattern='MM/dd/yyyy',
                                background = BG, foreground = FG, headersbackground = BG, headersforeground = DGR,
                                bordercolor = DGR,normalbackground = BG, normalforeground = FG, 
                                weekendbackground = BG, weekendforeground = FG,
                                selectbackground = DGR, selectforeground = 'black',
                                othermonthforeground = 'dim gray', othermonthbackground = BG, 
                                othermonthweforeground = 'dim gray', othermonthwebackground = BG, 
                                mindate=mindate, maxdate=maxdate, disabledforeground='red', font = fsmall,
                                tooltipbackground = BG, tooltipforeground = DGR )
        
        self.datepick.grid(row = 1, column = 0, padx = 5, pady = 5, sticky = tk.EW)
        
        self.datepick.bind('<<DateEntrySelected>>', self.check_if_monday)
        
        
        self.combob = ttk.Combobox(self.oth_months_instruct, values = self.choices)
        self.combob.grid(row = 1, column = 1, padx = 5, pady = 5)
        
        self.file_button = tk.Button(self.oth_months_instruct, text = 'Open DB', bg = BG, fg = FG,
                                     command = lambda: self.file_ask_open())
        self.file_button.grid(row = 1, column = 2, padx = 3, pady = 3, sticky = tk.EW)
        ...
        style.theme_use('alt')
        #style = ttk.Style()
        #style.configure("mystyle.Treeview", highlightthickness=0, bd=0, font=fsmall,
                        #background = 'black', foreground = 'white') # Modify the font of the body
        #style.configure("mystyle.Treeview.Heading", font=fbig, background = 'black', foreground = 'white') # Modify the font of the headings
        #style.configure("mystyle.Treeview.Cell", background = BG, foreground = FG)
        #style.configure('.', font = fsmall, background = BG, foreground = FG)
        ...
        self.tview = ttk.Treeview(self.bottomframe, columns = ('c1','c2','c3','c4','c5','c6','c7'), height = 5, style = "mystyle.Treeview")
        self.tview['columns'] = ('c1','c2','c3','c4','c5','c6','c7')
        self.tview.heading('c1', text = 'Account')
        self.tview.heading('c2', text = 'Contacts')
        self.tview.heading('c3', text = 'Date Last')
        self.tview.heading('c4', text = 'Profile Page')
        self.tview.heading('c5', text = 'ToDo')
        self.tview.heading('c6', text = 'Quote_Proj')
        self.tview.heading('c7', text = 'Procurement')
        ...
        self.tview.column('c1', width = 75, anchor = 'center')
        self.tview.column('c2', width = 100, anchor = 'center')
        self.tview.column('c3', width = 100, anchor = 'center')
        self.tview.column('c4', width = 125, anchor = 'center')
        self.tview.column('c5', width = 125, anchor = 'center')
        self.tview.column('c6', width = 155, anchor = 'center')
        
        
        self.tview.insert('','0','AZ', text = 'Arizona')
        self.tview.insert('','1','NV', text = 'Nevada')  
        self.tview.insert('','2','NM', text = 'New Mexico')
        
        #self.tview.bind('<<TreeviewOpen>>', self.grab_tview_select)
        self.tview.bind("<Double-1>", self.grab_tview_select)
        ...
        with open(acccon, 'r+')as F:
            lines = F.readlines()
            for line in lines:
                cline = line.split(',')
                print(cline[3])
                self.det_text.insert(tk.END, '{}\n'.format(cline))
                if cline[3] == 'AZ':
                    self.tview.insert('AZ', 'end', values = (cline[0],cline[1],cline[2],cline[3],cline[4],cline[5],cline[6]))
                if cline[3] == 'NV':
                    self.tview.insert('NV','end', values = (cline[0],cline[1],cline[2],cline[3],cline[4],cline[5],cline[6]))
                if cline[3] == 'NM':
                    self.tview.insert('NM','end', values = (cline[0],cline[1],cline[2],cline[3],cline[4],cline[5],cline[6]))                
                
        
        self.tview.grid(row = 0, columnspan = 5, padx = 5, pady = 5, sticky = tk.EW)      
        
        
    #----------------------------------------------------------------------------------------------
    def grab_tview_select(self, event):
        """
        try:
            print(repr(self.tview.selection_get()))
        
        except Exception as e:
            print(e)
        """   
        
        item = self.tview.selection()
        #print(repr(item[0]))
        #item = self.tview.identify('item', event.x, event.y)
        #for i in item:
            #print("you clicked on", self.tview.item(i, "values")[:]) 
        
        ents = self.tview.item(item[0], "values")[:]
        print(len(ents))
        dicthold = {'Account':ents[0],
                    'Contacts':ents[1],
                    'Date Last':ents[2],
                    'Profile Page':ents[3],
                    'ToDo':ents[4],
        ...
        #for i in ents:
            #itms = """Account: {}""".format(i)
            #print(itms)
            
        for k,v in dicthold.items():
            print('{} = {}'.format(k,v))
        
        

    #----------------------------------------------------------------------------------------------
    def file_ask_open(self):
        """
        ...
        fopen = fd.askopenfilename()
        c = sq3.connect(fopen)
        cc = c.cursor()
        
        #THIS IS HOW TO FIND OUT TABLE NAMES ---------------------------
        cc.execute("select * from SQLite_master")
        tables = cc.fetchall()        
        print("Listing tables and indices from main database:")
        ...
        cc.execute('SELECT * FROM caldates')
        with open(acccon, 'r+')as F:
            lines = F.readlines()
            for line in lines:
                self.det_text.insert(tk.END, '{}\n'.format(line))
    ...
    #----------------------------------------------------------------------------------------------
    def check_if_monday(self, event):
        """
        Called from event of Date entry widget. Check if date selected is a monday.
        """
        
        dater = event.widget.get_date()
        
        if dater.weekday() != 0:
            print('Pick a monday')
            
            #self.datepick.config(foreground = 'red')
        else:
            print(str(dater))
    ...
    def compchg(self, event):
        """
        Altering of the label "company" to indicate it has focus
        """
    ...
    def contchg(self, event):
        """
        Altering of the label "contacts" to indicate it has focus
        """
    ...
    def medchg(self, event):
        """
        Altering of the label "medium" to indicate it has focus
        """
    ...
    def test_print(self, day_of_week):
        ...
        self.det_text.config(state = tk.NORMAL)             #Turn the state of the text widget back on
    ...
    def display_widge_shrink(self, event):
        ...
        if d1['row'] == chk['row']:
            self.desclab.config(fg = DGR, text = 'DESC', font = 14)
            self.fwdlab.config(text = 'Forward', fg = FG, font = fsmall)
            
        else:
            self.fwdlab.config(fg = DGR,  text = 'FWD', font = 14)
            self.desclab.config(text = 'Description', fg = FG, font = fsmall)
    ...
    def write_to_file(self):
        ...
        tstr = 'Testing.txt'
        self.wfile_lab = tk.Label(self.wfile,  bg = BG, fg = FG, font = fbig,
